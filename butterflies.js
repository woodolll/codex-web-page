...
export function createButterflies(scene, { screen = false, reduced = false, camera = null } = {}) {
  const history=Array.from({length:18},()=>new THREE.Vector2(.5,.5));
  ...
  const flock = Array.from({ length: screen ? 22 : 18 }, (_, i) => {
    ...
    const material = new THREE.MeshStandardMaterial({color: i % 3 ? '#f3eee9' : '#bbc9d3', side: THREE.DoubleSide, roughness: .55, metalness: .1, transparent: true, opacity: screen ? .72 : .9, depthWrite: false, depthTest: false});
  ...
  return time => {
    ...
    history[0].lerp(pointer,1-Math.exp(-dt*28));
    for(let j=history.length-1;j>0;j--)history[j].lerp(history[j-1],1-Math.exp(-dt*55));
    flock.forEach(({group,left,right,phase,speed}, i) => {
      ...
      if(seen&&!reduced){
        const target=history[Math.min(17,Math.floor(i*.7))];
        ...
        group.userData.follow.lerp(destination,1-Math.exp(-dt*(8+(i%4)*1.2)));
        group.position.copy(group.userData.follow);
      }else group.userData.follow=null;
      if(!screen&&camera){group.quaternion.copy(camera.quaternion);group.rotateZ(Math.cos(a*.71)*.35)}else group.rotation.set(Math.sin(a*.6)*.35, Math.sin(a)*.6, Math.cos(a*.71)*.55);
