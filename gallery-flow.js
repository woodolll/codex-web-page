...
export function createGalleryFlow({reduced}){
 ...
 const vertex=`uniform vec2 size;uniform float bend;uniform float direction;uniform float side;uniform float time;varying vec2 vUv;varying float vCurl;
 void main(){vUv=uv;vec3 p=position;float u=mix(uv.x,1.-uv.x,step(side,0.));float edge=pow(smoothstep(.12,1.,u),1.8);float angle=edge*bend*2.7;float signX=side;
 ...
 p.z=(1.-cos(angle))*size.x*.22;
 float perspective=1000./(1000.-p.z);p.xy*=perspective;
 vCurl=edge*bend;gl_Position=projectionMatrix*modelViewMatrix*vec4(p,1.);}`;
 const fragment=`uniform sampler2D map;uniform float fade;uniform float bend;uniform float time;varying vec2 vUv;varying float vCurl;
 void main(){vec2 uv=vUv;uv.x+=sin(uv.y*22.+time*2.)*vCurl*.004;vec4 c=texture2D(map,uv);float sheen=sin(vCurl*3.14159)*.09;c.rgb=mix(c.rgb,vec3(.79,.88,.92),sheen);float edge=smoothstep(0.,.002,vUv.x)*smoothstep(0.,.002,1.-vUv.x);gl_FragColor=vec4(c.rgb,c.a*fade*edge);
 ...
 document.querySelectorAll('.look').forEach((el,i)=>{
  ...
  const material=new THREE.ShaderMaterial({uniforms:{map:{value:texture},size:{value:new THREE.Vector2()},bend:{value:0},direction:{value:1},side:{value:i%2?1:-1},time:{value:0},fade:{value:1}},vertexShader:vertex,fragmentShader:fragment,transparent:true,side:THREE.DoubleSide,depthTest:false,depthWrite:false});
 ...
 function frame(now){requestAnimationFrame(frame);const dt=Math.min((now-lastTime)/1000,.05);lastTime=now;if(page.hidden||document.hidden){canvas.style.display='none';lastScroll=page.scrollTop;velocity=0;return}canvas.style.display='block';time+=dt;
  ...
  items.forEach((item,i)=>{const {el,mesh}=item;if(!el.hidden&&!item.ready)item.paint();if(el.hidden||!item.ready){mesh.visible=false;return}const r=el.getBoundingClientRect();if(Math.abs(r.width-item.width)>1)item.paint();mesh.visible=r.bottom>header-150&&r.top<h+180;if(!mesh.visible)return;
   ...
   const exit=THREE.MathUtils.smoothstep(header+85-r.bottom,0,170);const entering=THREE.MathUtils.smoothstep(r.top-(h-70),0,170);
   ...
   uniforms.bend.value+=(bend-uniforms.bend.value)*(1-Math.exp(-dt*12));uniforms.direction.value=velocity<-.02?-1:1;uniforms.time.value=time;uniforms.fade.value=1-exit*.94;
  });
